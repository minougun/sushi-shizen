...
| **住所の文言・地図 URL の統一** | `index.html`（`dd` / フッター / Maps リンク / 埋め込み iframe）、`js/main.js` の `address_access_dd`・`footer_address`・`REVIEW_DEFAULTS`・`map_frame_title` 等、**4 言語すべて**。必要なら `review-scores.json` / `api/review-scores.js` / `public-mock.*` |
| **スマホで住所1行を折り返さない** | `css/style.css` の `.address__line` / `.address__subline` と `.body__ja …` の `clamp`（ご案内・フッター） |
...
## 住所の表示とモバイル最適化（2026-04 追記）

### 方針

- **建物住所の1行目**を、狭い画面でも **意味の途中で改行されにくく**するため、住所だけを `<span class="address__line">` で囲む。
- **アクセスの補足**（例: 「心斎橋駅 徒歩5分」）は `<span class="address__subline">` に分け、通常どおり折り返し可（`white-space: normal`）。
- フッターは **建物住所のみ**なら `.address__line` だけでよい。

### マークアップ例（`index.html` のフォールバック・`data-i18n-html`）

**ご案内（`#informationSection`）の `address_access_dd`:**

```html
<dd data-i18n-html="address_access_dd">
  <span class="address__line">大阪府大阪市中央区東心斎橋1-14-15 アルスビル 4F</span><br>
  <span class="address__subline">心斎橋駅 徒歩5分</span>
</dd>
```

**フッターの `footer_address`:**

```html
<p class="content__footer_address" data-i18n-html="footer_address">
...
- `address_access_dd` / `footer_address` は **`data-i18n-html` 用**のため、**`ja` / `en` / `ko` / `zh` すべて**で **同じ HTML 構造**（`span.address__line` / `address__subline`）を保つ。
- 言語切替後も DOMPurify でサニタイズされるが、`span` + `class` は通常そのまま通る。構造を変える場合はブラウザで各言語を確認する。

### CSS（`css/style.css`）

- **共通（`.body__ja`）**  
  - `.address__line` … `white-space: nowrap`、`word-break: keep-all` などで **日本語の1行を維持**しつつ、はみ出しを抑える。
  - `.address__subline` … 通常の折り返し。
- **767px 以下**（スマホレイヤー内）  
  - `#informationSection .address__line` と `.content__footer_address .address__line` に **`font-size: clamp(...)`** を指定し、**ビューポート幅に応じて字詰め**（`100vw` と `var(--sz-safe-x)` / `var(--sz-safe-xr)` を式に含む）。

英語表示（`.body__en` 等）では **`nowrap` を掛けない**想定。長い英住所は折り返し優先。

### 住所を変えたときに揃える場所（漏れ防止）

表示文字列だけでなく、**地図・リンクのクエリ**も同一住所に合わせる。

| 種別 | 場所の例 |
|------|-----------|
| Google Maps 検索 URL | `index.html` 内 `data-google-maps-link` 付き `<a>`（ヒーロー・予約・ご案内） |
| 埋め込み地図 | `index.html` の `<iframe src="https://www.google.com/maps?q=...">` |
| 口コミ API フォールバック | `js/main.js` の `REVIEW_DEFAULTS.google.url`、`review-scores.json`、`api/review-scores.js` のデフォルト URL |
| モック | `public-mock.html` / `js/public-mock.js`（任意で本番に合わせる） |

URL は **パーセントエンコード**されたままの行が多いので、エディタの「一括置換」より **1箇所ずつデコードして確認**するか、地図画面で住所を検索し直して URL を取り直すと安全。

### 本番反映まわり（運用メモ）

- `git push` だけでは Vercel 本番が即座に変わらない構成（Git Disconnect 運用）のときは、**必ず** リポジトリルートで `npm run deploy:prod` を実行する。
- 一時ディレクトリから誤って **別プロジェクト名**で `vercel --prod` すると、意図しないデプロイ先になる。**`.vercel/project.json`**（`projectName`: `sushi-zen` 等）が正しいディレクトリで実行する。
- 反映確認: `curl -sL https://sushi-zen.net | rg address__line` のように、公開 HTML に **新マークアップ**が載ったか確認できる。
...
- [ ] 住所を変更したら **`address_access_dd` / `footer_address`（4言語・HTML構造）** と **`index.html` の Maps リンク・iframe**・**`REVIEW_DEFAULTS` / `review-scores.json`** を揃える
...
- **住所** … 統一表記「大阪府大阪市中央区東心斎橋1-14-15 アルスビル 4F」。**`.address__line` / `.address__subline`** でスマホ時の1行表示と流体 `font-size` を実装
...
9. **住所まわり** … `address_access_dd` / `footer_address` を HTML ラップ + 全言語同期。Google Maps の `query` / 埋め込み `q=` / 口コミ用デフォルト URL を同じ住所に合わせる。本番は **`npm run deploy:prod`** で確実に反映（`.vercel/project.json` のプロジェクト名を確認）。
...
- 2026-04-06: **住所の統一（アルスビル 4F）**、**`.address__line` / `.address__subline` によるスマホ1行表示と流体字詰め**、**変更時の差し替え一覧（Maps・iframe・REVIEW_DEFAULTS 等）**、**本番反映（`deploy:prod` と project.json 注意）** を追記
